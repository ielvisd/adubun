...
This is a MANDATORY requirement - any frame showing skincare products without proper application sequence will be rejected. `
  ...
  if (hasReferenceImages) {
    ...
    // Place hook comparison FIRST (highest priority for CTA), then makeup/skincare application (if applicable), then body part prevention, then duplicate prevention, then mirror exclusion, then background consistency, then skin quality, then character visibility, then avatar, then character, then single product, then product consistency
    return `${hookComparisonInstruction}${makeupApplicationInstruction}${skincareApplicationInstruction}${bodyPartPrevention}${duplicatePrevention}${mirrorExclusionInstruction}${backgroundConsistencyInstruction}${skinQualityInstruction}${characterVisibilityInstruction}${avatarInstruction}${characterInstruction}${noTextInstruction}${previousFrameInstruction}${singleProductInstruction}${productConsistencyInstruction}${basePrompt}, ${moodStyle}, professional product photography, high quality, product must be pixel-perfect match to reference images, product appearance must be identical to reference images ${variationReinforcement}`
  } else {
    // Place hook comparison FIRST (highest priority for CTA), then makeup/skincare application (if applicable), then body part prevention, then duplicate prevention, then mirror exclusion, then background consistency, then skin quality, then character visibility, then avatar, then character, then single product
    return `${hookComparisonInstruction}${makeupApplicationInstruction}${skincareApplicationInstruction}${bodyPartPrevention}${duplicatePrevention}${mirrorExclusionInstruction}${backgroundConsistencyInstruction}${skinQualityInstruction}${characterVisibilityInstruction}${avatarInstruction}${characterInstruction}${noTextInstruction}${previousFrameInstruction}${singleProductInstruction}${basePrompt}, ${moodStyle}, professional product photography, high quality ${variationReinforcement}`
  }
}
