...
- For DIALOGUE:
  ...
  - Dialogue should progress the story: Hook introduces, Body1 develops, Body2 builds, CTA concludes. Each scene's dialogue should feel like a natural next step in the conversation or narrative.
...
🚨 REMEMBER: This is a 16-second format with 3 segments (6s + 6s + 4s). Each segment must be a single continuous shot with zero cuts. Match camera momentum between segments. End each clip on a mini-resolve. The CTA segment must end exactly at 16.000s.`
    ...
    if (segments.length !== 3) {
      // Fill in missing segments from story - use bodyOne for product intro, combine bodyTwo into CTA
      const baseSegments: Segment[] = [
        ...
        { type: 'body', description: story.bodyOne, startTime: 6, endTime: 12, visualPrompt: `${story.bodyOne}, professional ad quality, single continuous shot, no cuts, slow-motion reveal at 4-5s`, status: 'pending', audioNotes: '' },
        { type: 'cta', description: `${story.bodyTwo} ${story.callToAction}`, startTime: 12, endTime: 16, visualPrompt: `${story.bodyTwo} ${story.callToAction}, professional ad quality, single continuous shot, no cuts, ends at exactly 16.000s`, status: 'pending', audioNotes: '' },
      ]
